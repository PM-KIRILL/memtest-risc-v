/* **********************************************************
 * Copyright 2002 VMware, Inc.  Все права защищены. -- VMware Confidential
 * **********************************************************/
...
typedef struct {
   ...
   uint8_t apic_id;       /* Локальный номер APIC */
   uint8_t apic_ver;      /* Его версии */
   uint8_t cpu_flag;
#define CPU_ENABLED             1       /* Процессор доступен */
#define CPU_BOOTPROCESSOR       2       /* Процессор является загрузочным (BP) */
   uint32_t cpu_signature;
#define CPU_STEPPING_MASK 0x0F
#define CPU_MODEL_MASK  0xF0
#define CPU_FAMILY_MASK 0xF00
   uint32_t featureflag;  /* Значение признаков CPUID */
   uint32_t reserved[2];
} mp_processor_entry_t;

typedef struct {
   uint8_t type;   // имеет значение MP_BUS
   uint8_t busid;
   char  bustype[6];
} mp_bus_entry_t;
...
typedef struct {
   uint8_t  type;   // установлено в MP_IOAPIC
   uint8_t  apicid;
   uint8_t  apicver;
   uint8_t  flags;
...
/* Типы триггеров APIC (по фронту/по уровню) */
#define APIC_TRIGGER_EDGE     0
#define APIC_TRIGGER_LEVEL    1

/* Режимы доставки APIC */
...
__GET_CPUID(int ax, uint32_t *regs)
{
   __asm__ __volatile__("\t"
   	/* сохраняем ebx на случай использования -fPIC */
...
static inline void spin_wait(spinlock_t *lck)
{
	if (cpu_id.fid.bits.mon) {
	    /* Используем monitor/mwait для ожидания с низким энергопотреблением и низкой состязательностью */
            asm volatile(
		"movl $0,%%ecx\n\t"
		"movl %%ecx, %%edx\n\t"
		"1:\n\t"
	    ...
	    /* monitor/mwait отсутствуют, поэтому просто ожидаем с использованием множества nop */
       	    int inc = 0x400;
            asm volatile(
		"1:\t"
		"cmpb $0,%1\n\t"
