...
static unsigned long ip_compute_csum(void *addr, unsigned long length)
{
	...
	unsigned long len;
	unsigned long laddr;
	/* вычисление контрольной суммы в стиле IP */
	laddr = (unsigned long )addr;
	sum = 0;
	if (laddr & 1) {
		uint16_t buffer;
		unsigned char *ptr;
		/* копирование первого байта в 2-байтовый буфер.
		 * Таким образом автоматически решается вопрос порядка байтов (endianness):
		 * в какой байт (младший или старший) попадает последний байт.
		 */
		buffer = 0;
		ptr = addr;
	...
	if (length & 1) {
		uint16_t buffer;
		unsigned char *ptr;
		/* копирование последнего байта в 2-байтовый буфер.
		 * Таким образом автоматически решается вопрос порядка байтов (endianness):
		 * в какой байт (младший или старший) попадает последний байт.
		 */
		buffer = 0;
...
static struct lb_header * find_lb_table(void)
{
	...
	head = 0;
	if (!head) {
		/* Сначала пробуем по адресу 0 */
		head = __find_lb_table(0x00000, 0x1000);
	}
	if (!head) {
		/* Затем пробуем по адресу 0xf0000 */
...
int query_linuxbios(void)
{
	...
	if (!head) {
	 ...
	 /* coreboot также может перенаправлять таблицу в область верхних таблиц. */
