...
unsigned char ich5_smb_read_byte(unsigned char adr, unsigned char cmd)
{
    ...
    __outb(0x1f, SMBHSTSTS);			// сброс контроллера SMBus
    __outb(0xff, SMBHSTDAT);
    while(__inb(SMBHSTSTS) & 0x01);		// ожидание готовности
    __outb(cmd, SMBHSTCMD);
    __outb((adr << 1) | 0x01, SMBHSTADD);
    __outb(0x48, SMBHSTCNT);
    rdtsc(l1, h1);
    //cprint(POP2_Y, POP2_X + 16, s + cmd % 8);	// индикатор прогресса
    while (!(__inb(SMBHSTSTS) & 0x02)) {	// ожидание завершения команды
			rdtsc(l2, h2);
			t = ((h2 - h1) * 0xffffffff + (l2 - l1)) / v->clks_msec;
			if (t > 10) break;			// прервать через 10 мс
    }
...
static int ich5_read_spd(int dimmadr)
{
    ...
    if (spd_raw[0] == 0xff)	return -1;		// здесь нет данных SPD
    for (x = 1; x < 256; x++) {
...
unsigned char us15w_smb_read_byte(unsigned char adr, unsigned char cmd)
{
    int l1, h1, l2, h2;
    unsigned long long t;
    //__outb(0x00, smbusbase + 1);			// сброс контроллера SMBus
    //__outb(0x00, smbusbase + 6);
    //while((__inb(smbusbase + 1) & 0x08) != 0);		// ожидание готовности
    __outb(0x02, smbusbase + 0);    // Чтение байта
    __outb(cmd, smbusbase + 5);     // Команда
    __outb(0x07, smbusbase + 1);    // Очистка статуса
    __outb((adr << 1) | 0x01, smbusbase + 4);   // Адрес DIMM
    __outb(0x12, smbusbase + 0);    // Запуск
    //while (((__inb(smbusbase + 1) & 0x08) == 0)) {}	// ожидание занятости
    rdtsc(l1, h1);
    cprint(POP2_Y, POP2_X + 16, s + cmd % 8);	// индикатор прогресса
    while (((__inb(smbusbase + 1) & 0x01) == 0) ||
		((__inb(smbusbase + 1) & 0x08) != 0)) {	// ожидание завершения команды
	rdtsc(l2, h2);
	t = ((h2 - h1) * 0xffffffff + (l2 - l1)) / v->clks_msec;
	if (t > 10) break;			// прервать через 10 мс
    }
...
int find_smb_controller(void)
{
    ...
    for (smbdev = 0; smbdev < 32; smbdev++) {
		    ...
		    if (valuev != 0xFFFF) {					// если что-то найдено, проверяем, что это...
					for (i = 0; smbcontrollers[i].vendor > 0; i++) {	// проверяем, является ли это известным контроллером SMBus
			    	if (valuev == smbcontrollers[i].vendor) {
							pci_conf_read(0, smbdev, smbfun, 2, 2, &valued);	// чтение ID устройства
...
void get_spd_spec(void)
{
    ...
    if (index == -1) 
    {
    	// Неизвестный контроллер SMBUS, выход
			return;
    }
    ...
    for (j = 0; j < 8; j++) {
			if (smbcontrollers[index].read_spd(j) == 0) {	
				curcol = 1;
				if(spd_raw[2] == 0x0b){
				  // Мы здесь, если обнаружена память DDR3
				 
				  // Сначала выводим номер слота и емкость модуля
					...
					// Если XMP поддерживается, проверяем Tck в регистрах XMP					
					...
					// Затем выводим информацию о модуле (производитель и номер детали)	
					spd_raw[117] &= 0x0F; // Четность или нечетность
					for (i = 0; jep106[i].cont_code < 9; i++) {	
			    	if (spd_raw[117] == jep106[i].cont_code && spd_raw[118] == jep106[i].hex_byte) {
			    		// Мы здесь, если обнаружен производитель JEDEC
							...
							// Определение недели и года производства (не забудьте обновить после 2015 года !!!)
			...
			// Вход в эту функцию при обнаружении DDR2
			if(spd_raw[2] == 0x08){				
					 // Сначала выводим номер слота и емкость модуля
					cprint(LINE_SPD+k, curcol, " - Slot   :");
					dprint(LINE_SPD+k, curcol+8, k, 1, 0);
