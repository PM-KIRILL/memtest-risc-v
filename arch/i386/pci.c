Вот полный код с переведенными комментариями:

```c
/* pci.c - MemTest-86  Версия 3.2
 *
 * Выпущено под лицензией Gnu Public License версии 2.
 * Автор: Крис Брэди (Chris Brady)
 * ----------------------------------------------------
 * Специфический код MemTest86+ V5.00 (GPL V2.0)
...
static int pci_sanity_check(void)
{
	unsigned long value;
	int result;
	/* Выполнить простую проверку, чтобы убедиться, что мы видим хост-мост (host bridge).
	 * Сообщается, что в некоторых чипсетах от Intel и
	 * Compaq есть ошибки, из-за которых этот тест не работает. Я займусь
	 * этим, когда мы добавим их поддержку.
	 */
...
static int pci_check_direct(void)
{
	...
	if (cpu_id.vend_id.char_array[0] == 'A' && cpu_id.vers.bits.family == 0xF) {
			...
			/* Проверить, работает ли тип конфигурации 1. */
			pci_conf_type = PCI_CONF_TYPE_1;
			tmpCFB = inb(0xCFB);
			outb(0x01, 0xCFB);
			tmpCF8 = inl(0xCF8);
			...
			/* Проверить, работает ли тип конфигурации 2. */
	...
	/* Ничего не сработало, вернуть ошибку */
...
{
	int result;
	/* На данный момент просто убедиться, что мы можем напрямую
	 * использовать функции PCI.
	 */
