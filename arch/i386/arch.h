...
struct e820entry {
        unsigned long long addr;        /* начало сегмента памяти */
        unsigned long long size;        /* размер сегмента памяти */
        unsigned long type;             /* тип сегмента памяти */
};
...
/*
 * Проверено по руководству Intel и GCC --hpreg
 *
 * volatile, так как tsc всегда изменяется без ведома компилятора.
 */
...
/* передача командной строки с использованием «старого» протокола загрузки */
