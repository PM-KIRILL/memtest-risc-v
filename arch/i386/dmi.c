/* dmi.c использует DMI из SMBIOS для чтения информации о возможностях 
 * аппаратных устройств памяти и о том, где они отображены в адресном пространстве
 *
 * Copyright (c) Joachim Deguara, AMD 2006
 *
 * Выпущено под лицензией GPL версии 2
 * ----------------------------------------------------
 * Memtest86+ V4.00 - Добавлено соответствие спецификации SMBIOS V2.6.1
 */
...
int open_dmi(void){
	...
	// нам нужна как минимум версия 2.1 SMBIOS
...
// просмотр всех структур
	while(dmi < table_start + eps->tablelength){
		...
		// Требуется исправление (SMBIOS/DDR3)
...
void print_dmi_startup_info(void)
{
	...
	if(sl1 && sl2)
		{
			//dmicol -= slenght; // выравнивание по правому краю
			dmicol = 39 - slenght/2; // выравнивание по центру
...
void print_dmi_info(void){
	...
	for(page=1; page <= 1 + (mem_devs_count-1)/8; page++){
		...
		for(i=8*(page-1); i<mem_devs_count && i<8*page; i++){
			...
			// это единственное поле, для которого требуется SMBIOS 2.3+ 
			if ( mem_devs[i]->speed && 
...
// возвращает 1, если список неисправных устройств памяти изменился, 0 в противном случае, -1 если не отображено
int add_dmi_err(ulong adr){
	...
	for(i=0; i < md_maps_count; i++){
		...
		// найдено соответствующее отображение, теперь ищем соответствующее устройство
