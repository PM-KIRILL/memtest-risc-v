...
static void parse_fdt(uint8_t *dtb)
{
	...
	int cont = 1;
	while (cont) {
		// выравнивание по 4-байтовой границе
...
void serial_echo_init(void)
{
	...
	// Пока не меняем скорость передачи данных, используем 115200 по умолчанию
}
...
static void move_to_correct_addr(void)
{
	...
	if (cur_start == initial_load_addr &&
		...
		// Необязательное первое перемещение, перенос чуть дальше начальной позиции
		// **Необязательно**, так как мы не можем выполнить это при загрузке по самым высоким адресам
// 		serial_echo_print("FIRST STARTUP RELOCATION...\n");
		uintptr_t temp_addr = (((uintptr_t)&_end >> 12) + 1) << 12;
		run_at(temp_addr, 0);
	} else {
		// финальное перемещение при запуске
		// теперь у нас **точно** есть свободное место в начале
...
void riscv_entry(uint64_t hart_id, uint64_t fdt_address)
{
	...
	if (start_seq < 2) {
		...
		/* Устанавливаем адрес перемещения на 32 МБ, если достаточно
			* памяти. В противном случае устанавливаем его на 3 МБ */
		/* Большой адрес перемещения позволяет большее перекрытие при тестировании */
	...
	/* Барьер для гарантии того, что все CPU завершили запуск */
	barrier();
	 ...
	 * потому что нам нужны все доступные CPU */
	if (start_seq < 2) {

	    btrace(my_cpu_num, __LINE__, "Mem Mgmnt ", 1, 0, 0);
	    /* Получение скорости памяти со всеми CPU */
	    get_mem_speed(my_cpu_num, 1);
	}

	/* Устанавливаем флаг инициализации только после того, как все CPU
	 * достигли барьера. Это гарантирует, что перемещение было
	 * завершено для каждого CPU. */
...
// Из "The RISC-V Instruction Set Manual, Volume II: Privileged Architecture, Document Version 20190608-Priv-MSU-Ratified"
