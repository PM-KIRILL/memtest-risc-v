// основано на reloc.c из MemTest86+ v5.1

/* Возвращает адрес _DYNAMIC во время компоновки. По счастливому совпадению, это
   первый элемент GOT. Эта функция должна быть встроена (inlined) в функцию, которая
...
/* Выполняет релокацию, указанную в RELOC и SYM (который уже полностью разрешен).
		 ...
		 const ElfW(Sym) *sym, void *const reloc_addr)
{
	...
	value = sym->st_value;
	/* Каждая секция, кроме неопределенной, имеет базовый адрес map->l_addr */
	...
	case R_RISCV_COPY:
	{
		/* Реализуем memcpy вручную, так как вызовы функций пока недоступны. */
