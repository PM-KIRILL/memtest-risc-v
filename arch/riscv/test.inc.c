// Эти фрагменты создают довольно высокую нагрузку на регистры, но...
// * У меня их много :)
// * Я бы предпочел начать с чего-то простого
// * Я ожидаю, что вход в эти фрагменты и выход из них будут происходить довольно редко,
//   и вместо этого их выполнение будет занимать довольно много времени
...
OPTIMIZED_SNIPPET void movinvr_snippet1(uint32_t *p, uint32_t *pe, unsigned me)
{
	asm __volatile__ (
		...
		: : "r" (p), "r" (pe), "r"(me) : "a0", "a1" /* может содержать возвращаемое значение */, "s0", "s1", "s2"
	);
}
...
OPTIMIZED_SNIPPET void movinv32_snippet1(
	int *p_k, uint32_t *p_pat, // входные-выходные данные
	uint32_t *p, uint32_t *pe, uint32_t sval, uint32_t lb // только входные данные
)
{
	uint32_t k = *p_k;
	uint32_t pat = *p_pat;
...
OPTIMIZED_SNIPPET void movinv32_snippet3(
	int *p_k, uint32_t *p_pat, // входные-выходные данные
	uint32_t *p, uint32_t *pe, uint32_t p3, uint32_t hb // только входные данные
)
{
	uint32_t k = *p_k;
	uint32_t pat = *p_pat;
...
OPTIMIZED_SNIPPET void modtst_snippet1(
	...
	uint32_t *pe, uint32_t p1 // только входные данные
)
{
	uint32_t *p = *p_p;
...
OPTIMIZED_SNIPPET void modtst_snippet2(
	int *p_k, // вход-выход
	uint32_t *p, uint32_t *pe, uint32_t p2, int offset // только входные данные
)
{
	int k = *p_k;
