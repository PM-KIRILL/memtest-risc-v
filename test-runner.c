/*
 * Это загрузчик пользовательского пространства для различных тестов памяти.
 * Его можно использовать для упрощения отладки оптимизированных
 * реализаций тестов.
 */
...
// Пока не включаем stdlib.h из-за конфликтов
void abort(void);
...
#if !HAS_FLAT_MEM // в противном случае они уже определены
...
// выделение в нижней половине адресного пространства
#define VMEM_SZ (1 << 20)
...
{
	// выравнивание по границе следующей страницы
...
int main(int argc, const char *argv[])
{
	...
	/*
	 * Используем как минимум две итерации, чтобы snippet3() проверялся относительно snippet2()
	 * в подобных случаях:
	 *
	 * snippet1();
