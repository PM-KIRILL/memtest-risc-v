/* screen_buffer.c - MemTest-86  Версия 3.3
 *
 * Выпущено под лицензией Gnu Public License версии 2.
 * Автор: Jani Averbach, Jaa@iki.fi, 2001
 */
...
/*
 * Размер по X должен быть на единицу больше размера экрана, 
 * чтобы осталось место для завершающего символа '\0'
 */
	...
	int y, int x, const char *text)
{
	...
	/* Если делать больше нечего, возвращаемся */
	if (*text == '\0') {
		return;
	}
	ttyprint(y, x, text);
