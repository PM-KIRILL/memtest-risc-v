/* config.c - MemTest-86 Версия 3.4
 *
 * Выпущено под лицензией Gnu Public License версии 2.
 * Автор: Крис Брэди (Chris Brady)
 * ----------------------------------------------------
 * Специфичный код MemTest86+ V5.00 (GPL V2.0)
 * Автор: Самуэль ДЕМЁЛЬМЕСТЕР (Samuel DEMEULEMEESTER), sdemeule@memtest.org
...
void get_config()
{
  ...
  char cp[64];
	...
	while(!flag) {
		...
		/* Черт! Это портит ввод через последовательный порт. */
		sflag = 0;
		switch(get_key()) {
		case 2:
			/* 1 - Выбор тестов */
		  ...
		  cprint(POP_Y+6, POP_X+6, "(4) Enter Test List");
			...
			while (!sflag) {
				...
				case 4:
					...
					if (n <= 11) 
						{
					    /* Снять выделение со всех тестов */
					    i = 0;
					    while (tseq[i].cpu_sel) {
					        tseq[i].sel = 0;
					        i++;
					    }
					    /* Теперь устанавливаем выбор */
					    tseq[n].sel = 1;
					    v->pass = -1;
					    test = n;
					    find_ticks_for_pass();
				...
				"of tests to execute:");
					...
					/* Теперь включаем все тесты из списка */
					i = j = m = 0;
					while (1) {
					    if (isdigit(cp[i])) {
						n = cp[i]-'0';
					...
					/* Если не выбрано ни одного теста, 
					 * включаем их все обратно */
					if (m == 0) {
					    k = 0;
					    while (tseq[k].cpu_sel) {
					        tseq[k].sel = 1;
		...
		case 4:
			...
			while (!sflag) {
				...
				case 5:
					...
					break;
				case 6:
					/* Установить режим звукового сигнала при ошибке */
					beepmode = !beepmode;
					sflag++;
					break;						
				case 11:
		...
		case 6:
			...
			break;
		case 7:
			/* Отобразить информацию DMI о памяти */
			pop2up();
      print_dmi_info();
			pop2down();			
			break;
		case 8:
			/* Отобразить данные SPD */			
			popdown();
			show_spd();
			popup();
			sflag++;
	...
	for (i=POP_Y; i<POP_Y + POP_H; i++) { 
		for (j=POP_X; j<POP_X + POP_W; j++) { 
			pp = (char *)(SCREEN_ADR + (i * 160) + (j * 2));
                        save[0][i-POP_Y][j-POP_X] = *pp;  /* Сохранить экран */
                        set_scrn_buf(i, j, ' ');
			*pp = ' ';		/* Очистить */                        
			pp++;
                        save[1][i-POP_Y][j-POP_X] = *pp;
			*pp = 0x07;		/* Изменить фон на черный */
		}
	}
...
void popdown()
{
	...
	for (i=POP_Y; i<POP_Y + POP_H; i++) { 
		for (j=POP_X; j<POP_X + POP_W; j++) { 
			pp = (char *)(SCREEN_ADR + (i * 160) + (j * 2));
			*pp = save[0][i-POP_Y][j-POP_X]; /* Восстановить экран */
                        set_scrn_buf(i, j, save[0][i-POP_Y][j-POP_X]);
			pp++;
			*pp = save[1][i-POP_Y][j-POP_X]; /* Восстановить цвет */
		}
	}
        tty_print_region(POP_Y, POP_X, POP_Y+POP_H, POP_X+POP_W);
}
...
void pop2up()
{
	...
	for (i=POP2_Y; i<POP2_Y + POP2_H; i++) { 
		for (j=POP2_X; j<POP2_X + POP2_W; j++) { 
			pp = (char *)(SCREEN_ADR + (i * 160) + (j * 2));
			save2[0][i-POP2_Y][j-POP2_X] = *pp;  /* Сохранить экран */
			set_scrn_buf(i, j, ' ');
			*pp = ' ';		/* Очистить */
			pp++;
...
void pop2down()
{
	...
	for (i=POP2_Y; i<POP2_Y + POP2_H; i++) { 
		for (j=POP2_X; j<POP2_X + POP2_W; j++) { 
			pp = (char *)(SCREEN_ADR + (i * 160) + (j * 2));
			*pp = save2[0][i-POP2_Y][j-POP2_X]; /* Восстановить экран */
			set_scrn_buf(i, j, save2[0][i-POP2_Y][j-POP2_X]);
			pp++;
			*pp = save2[1][i-POP2_Y][j-POP2_X]; /* Восстановить цвет */
		}
	}
        tty_print_region(POP2_Y, POP2_X, POP2_Y+POP2_H, POP2_X+POP2_W);
}
