// Это дополнительные материалы, добавленные в memtest+ с сайта memtest.org
// Код Эрика Нельсона и Wee
// (Проверено без специфических для производителя оптимизаций перед добавлением)
/* extra.c -
 *
 * Выпущено под лицензией GNU General Public License версии 2.
 *
 */
...
static struct memory_controller mem_ctr[] = {
	...
	{ 0x8086, 0x3580,  0, change_timing_i852}, // Intel i852GM - i855GM/GME (Но не i855PM)
};
...
void find_memctr(void)  // По сути, копия функции find_controller
{
...
void amd64_option()
{
	...
	if (mem_ctr[ctrl].worked)
	{
		...
		while(!flag)
		{
			...
			case 4:
				popclear();
				// Количество обходов очереди чтения-записи
			...
			case 6:
				popclear();
				// Включение команды и адресации 2T
				cprint(POP_Y+3, POP_X+4, "Command rate:");
				cprint(POP_Y+5, POP_X+4, "(1) 1T "); // поддерживается только в ревизии CG и более поздних
			...
			case 8:
				popclear();
				// Задержка RAS-RAS (Active-to-Active)
...
/////////////////////////////////////////////////////////
// а теперь переходим к увлекательной части изменения таймингов... //
/////////////////////////////////////////////////////////
...
void change_timing_i852(int cas, int rcd, int rp, int ras) {
	...
	// Время активности RAS до предзаряда (tRAS)
	int1 = int2 & 0xF9FF;
...
void change_timing_i875(int cas, int rcd, int rp, int ras){
	...
	/* Прочитать базовый адрес MMR и определить указатель из регистра переполнения BAR6 */
...
void change_timing_amd64(int cas, int rcd, int rp, int ras) {
	...
	// Задержка CAS (tCAS)
	int1 = dramtlr | 0x7;	// некоторые изменения вызовут зависание системы, пробовал Draminit — безрезультатно
	if      (cas == 1) { dramtlr = int1 ^ 0x2; }   // cas 2.5
	else if (cas == 2) { dramtlr = int1 ^ 0x6; }
	else if (cas == 3) { dramtlr = int1 ^ 0x5; }
	else if (cas == 4) { dramtlr = int1 ^ 0x7; } // cas 1.5 на a64
	// иначе		   { dramtlr = dramtlr; }

//	pci_conf_read(0, 24, 2, 0x90, 4, &dramcr);// использовать dram init
	pci_conf_write(0, 24, 2, 0x88, 4, dramtlr);
	__delay(500);

////////////////////////////////////////////////////////////////
// попытка использования draminit, но не работает
}

// копия кода из lib.c для добавления задержки при модификации таймингов чипсета
...
void amd64_tweak(int rwt, int wrt, int ref, int en2t, int rct, int rrd, int rwqb, int wr)
{
	...
	if      (rrd == 2) { dramtlr = int1 ^ 0x50000; } // 2 такта шины
	else if (rrd == 3) { dramtlr = int1 ^ 0x40000; } // 3 такта шины
	else if (rrd == 4) { dramtlr = int1 ^ 0x30000; } // 4 такта шины
	// иначе		   { dramtlr = dramtlr;}

	// Время восстановления записи
	int1 = dramtlr | 0x10000000;
	if      (wr == 2) { dramtlr = int1 ^ 0x10000000; } // 2 такта шины
	else if (wr == 3) { dramtlr = int1 ^ 0x00000000; } // 3 такта шины
	// иначе		  { dramtlr = dramtlr;}
	...
	// Задержка записи перед чтением
	int1 = dramtlr | 0x1;
	if      (wrt == 2) { dramtlr = int1 ^ 0x0; }
	else if (wrt == 1) { dramtlr = int1 ^ 0x1; }
	// иначе		   { dramtlr = dramtlr;}

	// Задержка чтения перед записью
	int1 = dramtlr | 0x70;
	...
	// Частота обновления
	int1 = dramtlr | 0x1800;
	if      (ref == 1) { dramtlr = int1 ^ 0x1800; } // 15.6 мкс
	else if (ref == 2) { dramtlr = int1 ^ 0x1000; } // 7.8 мкс
	else if (ref == 3) { dramtlr = int1 ^ 0x0800; } // 3.9 мкс
	// иначе		   { dramtlr = dramtlr;}
